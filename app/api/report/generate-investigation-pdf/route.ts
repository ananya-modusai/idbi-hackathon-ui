...
export async function POST(req: Request) {
  try {
    ...
    // Normalize merchantIndustry to a plain string
    ...
    if (!/pdf-export-root-wrapper/.test(processedHtml)) {
      if (/<head[^>]*>/i.test(processedHtml)) {
        processedHtml = processedHtml.replace(/<head([^>]*)>/i, `<head$1><style>${watermarkCss}</style></head>`);
      } else {
    ...
    const getRiskStyles = (color: string) => {
      switch (color) {
        case "red": return { bg: "#FEF2F2", border: "#FECACA", text: "#B91C1C", iconBg: "#DC2626" };
        case "green": return { bg: "#ECFDF5", border: "#A7F3D0", text: "#047857", iconBg: "#059669" };
        case "purple": return { bg: "#F5F3FF", border: "#DDD6FE", text: "#6D28D9", iconBg: "#7C3AED" };
        case "orange": return { bg: "#FFF7ED", border: "#FED7AA", text: "#C2410C", iconBg: "#EA580C" };
        case "amber": return { bg: "#FFFBEB", border: "#FDE68A", text: "#B45309", iconBg: "#D97706" };
        default: return { bg: "#F9FAFB", border: "#E5E7EB", text: "#4B5563", iconBg: "#9CA3AF" };
      }
    };

    const riskStyles = getRiskStyles(riskColor);
    const checkIcon = `<svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#FFFFFF" stroke-width="4" stroke-linecap="round" stroke-linejoin="round" style="display:block;"><polyline points="20 6 9 17 4 12"></polyline></svg>`;
    const xIcon = `<svg xmlns="http://www.w3.org/2000/svg" width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="#FFFFFF" stroke-width="4" stroke-linecap="round" stroke-linejoin="round" style="display:block;"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>`;
    ...
    const headerTemplate = `
      <div style="font-family: Arial, Helvetica, sans-serif; width:100%; padding:0 10mm; box-sizing:border-box; margin:0 10mm; overflow:hidden;">
        <div style="display:flex; justify-content:space-between; align-items:center; width:100%; margin:0; padding:0;">
          <div style="color:#2563EB; font-weight:800; font-size:28px; letter-spacing:0.5px; line-height:1; margin:0; padding:0; display:flex; align-items:center; gap:12px;">
            ${String(merchantName).toUpperCase()}
            ${riskLabel ? `
              <div style="display:inline-flex; align-items:center; gap:5px; padding:2px 10px; border-radius:50px; border:1px solid ${riskStyles.border}; background-color:${riskStyles.bg}; color:${riskStyles.text}; font-size:11px; font-weight:800; text-transform:none; line-height:1.2; -webkit-print-color-adjust:exact;">
                <div style="width:14px; height:14px; min-width:14px; min-height:14px; border-radius:50%; background-color:${riskStyles.iconBg}; display:inline-flex; align-items:center; justify-content:center; -webkit-print-color-adjust:exact;">
                  ${riskLabel === "Not Recommended" ? xIcon : checkIcon}
                </div>
                <span style="white-space:nowrap;">${riskLabel}</span>
              </div>` : ''}
          </div>
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; width:100%; margin:0; padding:0;">
          <div style="color:#6B7280; font-size:14px; margin:0; padding:0; display:flex; align-items:center;">
            ${String(merchantIndustryLabel || '')} &nbsp;&nbsp; 
            ${website ? `<span style="margin-left:12px;"><a href="${website}" style="color:#2563EB; text-decoration:none;">${website}</a></span>` : ''} 
            ${riskScore && riskScore !== "N/A" ? `<span style="margin-left:16px; color:${riskStyles.text}; font-weight:700; display:inline-flex; align-items:center;">${clockIcon} Risk Score: ${riskScore}</span>` : ''}
            <span style="margin-left:16px;">Last Run Date: ${genDate}</span>
          </div>
          <div style="color:#6B7280; font-size:12px; font-weight:500; margin:0; padding:0;">Merchant Investigation Report</div>
        </div>

...
const BannedCategoryTab: FC = () => {
  const [data, setData] = useState<BannedCategoryDto[]>([]);
  ...
  const [selectedCategory, setSelectedCategory] = useState<BannedCategoryDto | null>(null);
  const [editContent, setEditContent] = useState("");
  const [addContent, setAddContent] = useState("");
  const [addType, setAddType] = useState<"BANNED" | "RESTRICTED">("BANNED");
  const [editType, setEditType] = useState<"BANNED" | "RESTRICTED">("BANNED");
  const [actionLoading, setActionLoading] = useState(false);

  // Sorting state
  const [sortDirection, setSortDirection] = useState<SortDirection>('desc');

  const loadBannedCategories = useCallback(async () => {
    try {
      setLoading(true);
      const response = await fetchBannedCategories("BANNED");
      if (response && response.success) {
        setData(response.data || []);
      }
    } catch (error) {
      console.error("Failed to fetch banned categories:", error);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleExportCSV = useCallback(() => {
    ...
    const headers = ["S.NO", "Banned Category", "Last Updated"];
    const csvContent = [
      headers.join(","),
      ...tableData.map(item => [
        item.serialNo,
    ...
    link.setAttribute("download", `banned_categories_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const handleEditClick = (category: BannedCategoryDto) => {
    setSelectedCategory(category);
    setEditContent(category.content);
    setEditType((category.type as any) === "RESTRICTED" ? "RESTRICTED" : "BANNED");
    setIsEditModalOpen(true);
  };

  const handleDeleteClick = (category: BannedCategoryDto) => {
    setSelectedCategory(category);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setAddContent("");
      setAddType("BANNED"); // Reset to default tab type
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        width: "200px",
        align: "center",
        render: (_: any, row: BannedCategoryDto) => (
          <div className="flex items-center gap-3">
            <Button
              variant="outline"
              size="sm"
        ...
        <DialogContent className="sm:max-w-[500px] p-0 overflow-hidden border-none shadow-2xl">
          ...
          <div className="px-6 py-4 bg-gray-50 flex justify-end gap-3 border-t border-gray-100">
            ...
            <Button
              ...
              disabled={actionLoading || !addContent.trim()}
            ...
            >
              {actionLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : "Add Category"}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
...
export default BannedCategoryTab;
